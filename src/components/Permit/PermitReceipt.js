...
export default function PermitReceipt() {
    const { id } = useParams();
    ...
    const sendReceiptToEmail = async () => {
        ...
        try {
            const response = await axios.post(
                `https://toogood-1.onrender.com/permit/send-receipt-email`,
                {
                    visaData: status,
                                        ...
                                        <div style={{ borderBottom: '2px solid #333', paddingBottom: '20px', marginBottom: '20px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                ...
                                                <div style={{ textAlign: 'right' }}>
                                                    ...
                                                    <p style={{ color: '#666', margin: 0 }}>#PER-{status.created_at ? new Date(status.created_at).toLocaleDateString() : "N/A"}</p>
                                                </div>
                                            </div>
                                        </div>
