...
const VisaSupportDetails = () => {
    ...
    const updatePayment = (visa_id, newPayStatus) => {
        axios.put(`https://toogood-1.onrender.com/admin/visa-payment-update/${visa_id}`, { payment_status: newPayStatus }, {
            headers: { Authorization: `Bearer ${token}` }
        })
        .then(() => {
            // Update the specific visa's payment status in the allVS array
            setAllVS(prevVS => prevVS.map(visa => 
        ...
        <>
            <div className='container d-md-flex gap-2'>
                <div className='col-12 col-md-12 overflow-scroll mb-4'>
                    <h6>List of Visa Support Applications</h6>
