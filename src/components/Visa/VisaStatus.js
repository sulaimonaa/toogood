...
const VisaStatus = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`https://toogood-1.onrender.com/visa/status/${id}`, {
            ...
            .catch(error => console.error("Error fetching approved visa details:", error))
            .finally(() => setLoading(false));

    }, [id, token]);

    const updatePayment = (visa_id, newPayStatus) => {
        axios.put(`https://toogood-1.onrender.com/admin/payment-update/${visa_id}`, { status: newPayStatus }, {
            headers: { Authorization: `Bearer ${token}` }
        })
            .then(() => {
    ...
    const visaStatus = (visa_id, newStatus) => {
        axios.put(`https://toogood-1.onrender.com/admin/visa-update/${visa_id}`, { status: newStatus }, {
            headers: { Authorization: `Bearer ${token}` }
        })
            .then(() => {
                setStatus(prevStatus => ({
                ...
                <div className='container'>
                    {loading ? (
                        ...
                        <div className='d-flex flex-column gap-2'>
                            ...
                            <div className='d-flex gap-0 align-items-center'>
                                <div className='bg-dark text-white p-2 w-25'>Tracking ID</div>
                                <div className='bg-secondary-subtle p-2 w-75'>
                                    {status?.tracking_id}
                                </div>
                            </div>
                            ...
                            <div className='d-flex gap-0 align-items-center'>
                                <div className='bg-dark text-white p-2 w-25'>Generate Receipt</div>
                                <div className='bg-secondary-subtle p-2 w-75'>
                                    <a href={`https://toogoodtravels.net/visa-receipt/${status?.id}`} target='_blank' rel="noopener noreferrer" className='text-decoration-none'>
                                        View/Send Receipt
                                    </a>
                                </div>
                            </div>
